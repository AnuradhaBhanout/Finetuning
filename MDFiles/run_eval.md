# usage: run_eval.py --label base --out eval/results_base.json
#        run_eval.py --compare eval/results_base.json eval/results_lora.json
# GEN_ENDPOINT env var for the RunPod/vLLM url, or swap generate() for local
...
from metrics import distinct_n, leaked_markup, length_ok, max_ngram_repeat, refusal_register, selfcheck
...
def build_prompt(character: str, situation: str) -> str:
    # has to match training + discord_bot.py exactly or the eval's testing nothing
    return (
